...
public interface ILevel{
    ...
    static double getTotalExpToLevel(double level) {
        double lv = Math.floor(level), x0 = ILevel.getTotalExpToFullLevel(lv);
        if (level == lv) return x0;
        return (ILevel.getTotalExpToFullLevel(lv + 1) - x0) * (level % 1) + x0;
    }
    ...
    static double getPercentageToNextLevel(double exp) {
        double lv = ILevel.getLevel(exp), x0 = getTotalExpToLevel(lv);
        return (exp - x0) / (ILevel.getTotalExpToLevel(lv + 1) - x0);
    }
}
